...
def display_top_games_by_score_type(df,feature, features, score_type, num_games=3):
    ...
    # Adjust aggregation to group by both 'title' and 'genre'
    aggregated_df = df.groupby(['title', feature]).agg({score_type: 'mean'}).reset_index()

    # Loop over each selected feature value and append the top games to the list
    ...
    # Combine all top games into a single DataFrame and reset the index
...
def prepare_data(df, classification_type, features = []):
    ...
    if classification_type == "type1":
        ...
        X_final = df[['metascore']] # Use Metascore as the only feature
...
def train_and_evaluate_model(X_train, X_test, y_train, y_test, model,type):
    ...
    # Evaluate the model based on the classification type
    if type =="type1":
